...
///
/// @brief equivalent to accessing a pointer by adding the index
///
/// example:
/// *CMat_pat(cmat, 1, 1) = 5;
///
...
///
/// @brief equivalent to accessing a pointer by adding the index than dereferencing
///
/// example:
/// CMat_at(cmat, 1, 1) = 5;
///
...
///
/// @brief create a matrix from an array without heap allocating
///
/// example:
...
///
/// @brief create a matrix from a 2d array without heap allocating
///
/// example:
...
///
/// @brief create a matrix from part of an array
///
/// warning:
/// DON'T do CMat_from_subarr(cmat->data, 0, 1, 2, 2);
...
/// @return a stack allocated matrix
///
#define CMat_from_subarr(arr, row_start, col_start, _nrow, _ncol)                                  \
    ((CMat){.data   = (CMatType *)(arr) + (row_start) * ((col_start) + (_ncol)) + (col_start),     \
            .nrow   = (_nrow),                                                                     \
            .ncol   = (_ncol),                                                                     \
            .stride = (col_start + _ncol)})
///
/// @brief create a matrix from part of another matrix
///
/// example:
/// CMatType arr[2][3] = {{1, 2, 3}, {4, 5, 6}};
/// CMat cmat = CMat_from_subarr(arr, 0, 1, 2, 2);
...
///
/// @brief populate an identity matrix into dst
///
/// example:
/// CMat identity;
...
///
/// @brief duplicate a matrix (allocated)
///
///
...
/// --   --
///
/// @param dst the resulted matrix; dst should have dst->nrow == src->ncol && dst->ncol ==
/// src->nrow
...
///
/// @brief do a dot product between 2 matrix and put the result into dst
///
/// example:
...
///
/// @brief cofactor of point row, col in matrix cmat
///
...
///
/// @brief determinant of matrix (laplace expansion)
///
/// example:
...
/// src->nrow == src->ncol && dst->nrow == src->nrow && dst->ncol == src->ncol
///
/// @param dst the destination of the inverse
/// @param src the matrix to get the inverse of
///
void CMat_inverse(CMat *dst, const CMat *src);

#ifndef CMAT_NO_PRINT
///
/// @brief print a matrix to the file f using the precision float_pres
///
...
///
/// @brief print a matrix to the file f using the precision CMAT_FPRINT_DEFAULT_PRES
///
/// @param f the file to print to
/// @param cmat the matrix to print
///
...
///
/// @brief print a matrix to stdout using the precision float_pres
///
/// @param cmat the matrix to print
/// @param float_pres the precision of the floating point value printing
///
...
///
/// @brief print a matrix to stdout using the precision CMAT_FPRINT_DEFAULT_PRES
///
/// @param cmat the matrix to print
///
...
void CMat_inverse(CMat *dst, const CMat *src) {
    CMatType det = CMat_det(src);
    CMAT_ASSERT(det != 0, "the determinant can't be 0");

    CMat_adj(dst, src);
    CMat_iterate(dst, row, col, val, *val /= det;);
}
...
void CMat_fprint_pres(FILE *f, const CMat *cmat, size_t float_pres) {
    ...
    for (size_t row = 0; row < cmat->nrow; ++row) {
        for (size_t col = 0; col < cmat->ncol; ++col) {
            ...
            // we don't print -0.0
            fprintf(f, "%*s%.*lf ", max_elems_size[col] - cur_elem_size, "", float_pres,
                    (cur_elem == -0.0) ? 0.0 : cur_elem);
        }
        fputs("|\n", f);
    }
    fprintf(f, "--%*s--\n", max_row_size, "");
